...
def create_record(
    name: str = Body(...), age: int = Body(...), description: str = Body(...)
):  # TODO: Refactor this
...
def remove_record(
    name: str = Path(...), age: int = Body(None), description: str = Body(None)
):
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Example fastapi backend")
    parser.add_argument(
        "--server-ip",
        dest="ip",
        type=str,
    ...
    if not (1 <= options.port <= 65535):
        raise ValueError(
            f"port number {options.port} is not in the allowed range for ports (1 to 65535)"
        )
